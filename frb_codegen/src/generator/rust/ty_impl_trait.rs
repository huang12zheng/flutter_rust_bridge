...
type_rust_generator_struct!(TypeImplTraitGenerator, IrTypeImplTrait);

impl TypeRustGeneratorTrait for TypeImplTraitGenerator<'_> {
    fn wire2api_body(&self) -> Acc<Option<String>> {
        Acc::new(|target| None)
    }
    // fn wire2api_body(&self) -> Acc<Option<String>> {
    //     let impl_trait = self.ir.get(self.context.ir_file);
    //     Acc::new(|target| {
    //         if matches!(target, Common) {
    //             return None;
    //         }
    //         let wasm = target.is_wasm();
    //         let mut variants = (impl_trait.variants())
    //             .iter()
    //             .enumerate()
    //             .map(|(idx, variant)| match &variant.kind {
    //                 IrVariantKind::Value => {
    //                     format!("{} => {}::{},", idx, impl_trait.name, variant.name)
    //                 }
